...
import net.changed.world.inventory.PurifierMenu;
...
public class PurifierBlockEntity extends BaseContainerBlockEntity implements StackedContentsCompatible {
    ...
    protected void loadAdditional(CompoundTag p_155025_, HolderLookup.Provider registries) {
    ...
    public static boolean isConversionRecipe(RecipeManager recipeManager, ItemStack stack) {
        return recipeManager.getAllRecipesFor(ChangedRecipeTypes.PURIFIER_RECIPE.get()).stream().anyMatch(recipe -> recipe.value().getIngredient().test(stack));
    }
    ...
    public ItemLike getConversionFor(RecipeManager recipeManager,ItemStack stack) {
        try {
            return recipeManager.getAllRecipesFor(ChangedRecipeTypes.PURIFIER_RECIPE.get()).stream()
                    .filter(recipe -> recipe.value().getIngredient().test(stack)).toList().get(0).value().getResult();
        } catch (Exception ex) {
