...
import net.changed.client.renderer.model.DuctPlayerModel;
...
public abstract class RenderOverride {
    ...
    public static class DuctPlayerOverride implements Override {
        ...
        public void render(Player player, @Nullable TransfurVariantInstance<?> variant, PoseStack pose, MultiBufferSource buffer, int packedLight, float partialTick) {
            ...
            playerModel.renderToBuffer(pose, buffer.getBuffer(RENDER_TYPE.apply(look)), packedLight, LivingEntityRenderer.getOverlayCoords(player, 0.0F), -1);
