...
import net.changed.client.latexparticles.LatexDripParticle;
import net.changed.client.latexparticles.SetupContext;
import net.changed.client.latexparticles.SurfacePoint;
...
public class LatexParticlesLayer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> implements FirstPersonLayer<T> {
    ...
    public void createNewDripParticle(LivingEntity entity) {
        ...
        if (model instanceof AdvancedHumanoidModel<?> advancedHumanoidModel)
            ...
            partsWithCubes = Stream.concat(
                    getAgeableParts(ageableListModel, "headParts"),
                    getAgeableParts(ageableListModel, "bodyParts")
    ...
    private static Stream<ModelPart> getAgeableParts(AgeableListModel<?> model, String methodName) {
        try {
            var method = AgeableListModel.class.getDeclaredMethod(methodName);
            method.setAccessible(true);
            @SuppressWarnings("unchecked")
            Iterable<ModelPart> parts = (Iterable<ModelPart>)method.invoke(model);
