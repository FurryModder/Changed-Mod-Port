...
public class LatexHumanoidArmorLayer<T extends ChangedEntity, M extends AdvancedHumanoidModel<T>> extends RenderLayer<T, M> {
    ...
    private void renderArmorPiece(PoseStack pose, MultiBufferSource buffers, T entity, EquipmentSlot slot, int packedLight, LatexHumanoidArmorModel<? super T, ?> model) {
        ...
        if (itemstack.getItem() instanceof ArmorItem) {
            ...
            if (armoritem.getEquipmentSlot() == slot) {
                var altModel = net.neoforged.neoforge.client.ClientHooks.getArmorModel(entity, itemstack, slot, model);
                if (altModel != model) {
    ...
    private void renderTrim(T entity, ItemStack stack, EquipmentSlot slot,
                            Holder<ArmorMaterial> material, PoseStack pose, MultiBufferSource buffers, int packedLight, ArmorTrim trim, LatexHumanoidArmorModel<? super T, ?> model, boolean inner) {
        model.prepareVisibility(slot, stack);
