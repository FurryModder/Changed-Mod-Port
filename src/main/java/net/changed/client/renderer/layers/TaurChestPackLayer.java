...
import net.changed.client.renderer.model.TaurChestPackModel;
import net.changed.data.AccessorySlots;
import net.changed.entity.ChangedEntity;
import net.changed.entity.beast.LatexTaur;
...
public class TaurChestPackLayer<T extends ChangedEntity & LatexTaur<T>, M extends AdvancedHumanoidModel<T> & LowerTorsoedModel> extends RenderLayer<T, M> {
    ...
    public void render(PoseStack pose, MultiBufferSource bufferSource, int i, T entity, float p_116670_, float p_116671_, float p_116672_, float p_116673_, float p_116674_, float p_116675_) {
        ...
        chestPackModel.renderToBuffer(pose, bufferSource.getBuffer(chestPackModel.renderType(chestPackModel.getTexture())), i, OverlayTexture.NO_OVERLAY, -1);
        pose.popPose();
