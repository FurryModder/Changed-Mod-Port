...
import net.changed.client.renderer.animate.multihead.DoubleHeadAnimator;
import net.changed.client.renderer.model.DoubleHeadedModel;
...
public class ArmorLatexMaleDoubleHeadedWingedDragonModel<T extends ChangedEntity & DoubleHeadedEntity> extends LatexHumanoidArmorModel<T, ArmorLatexMaleDoubleHeadedWingedDragonModel<T>> implements DoubleHeadedModel<T> {
    ...
    public void renderForSlot(T entity, RenderLayerParent<? super T, ?> parent, ItemStack stack, EquipmentSlot slot, PoseStack poseStack, VertexConsumer buffer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        ...
        switch (slot) {
            case HEAD -> {
                Head.render(poseStack, buffer, packedLight, packedOverlay, net.minecraft.util.FastColor.ARGB32.colorFromFloat(alpha, red, green, blue));
                Head2.render(poseStack, buffer, packedLight, packedOverlay, net.minecraft.util.FastColor.ARGB32.colorFromFloat(alpha, red, green, blue));
            }
            case CHEST -> {
