...
public class ChangedShaders {
    ...
    private static void setupWaveVisionShader(ShaderInstance shader) {
        getUniform(shader, "WaveEffect").ifPresent(uniform -> uniform.set(ChangedClient.getWaveEffect()));
    }

    private static void setupWaveVisionResonantShader(ShaderInstance shader, Vector3f resonance) {
        setupWaveVisionShader(shader);
        getUniform(shader, "WaveResonance").ifPresent(uniform -> uniform.set(resonance));
    }
    ...
    protected static final RenderStateShard.ShaderStateShard RENDERTYPE_WAVE_VISION_ENTITY_SHADER = new PrefixedShaderShard(ChangedShaders::getWaveVisionEntityShader, ChangedShaders::setupWaveVisionShader);
    protected static final RenderStateShard.ShaderStateShard RENDERTYPE_WAVE_VISION_SOLID_SHADER = new PrefixedShaderShard(ChangedShaders::getWaveVisionSolidShader, ChangedShaders::setupWaveVisionShader);
    protected static final RenderStateShard.ShaderStateShard RENDERTYPE_WAVE_VISION_CUTOUT_SHADER = new PrefixedShaderShard(ChangedShaders::getWaveVisionCutoutShader, ChangedShaders::setupWaveVisionShader);
    protected static final RenderStateShard.ShaderStateShard RENDERTYPE_WAVE_VISION_CUTOUT_MIPPED_SHADER = new PrefixedShaderShard(ChangedShaders::getWaveVisionCutoutMippedShader, ChangedShaders::setupWaveVisionShader);
    ...
    protected static final Function<Vector3f, RenderStateShard.ShaderStateShard> RENDERTYPE_WAVE_VISION_RESONANT_ENTITY_SHADER = Util.memoize(resonance -> {
        return new PrefixedShaderShard(ChangedShaders::getWaveVisionResonantEntityShader,
                shader -> setupWaveVisionResonantShader(shader, resonance));
    });
