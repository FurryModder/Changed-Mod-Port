package net.changed.client.tfanimations;

import com.mojang.blaze3d.vertex.PoseStack;
import com.mojang.datafixers.util.Pair;
import it.unimi.dsi.fastutil.objects.Object2ObjectArrayMap;
import net.changed.client.FormRenderHandler;
import net.changed.client.PoseStackExtender;
import net.changed.client.animations.AnimationContainer;
import net.changed.client.animations.Limb;
import net.changed.client.renderer.AdvancedHumanoidRenderer;
import net.changed.client.renderer.accessory.TransitionalAccessory;
import net.changed.client.renderer.layers.AccessoryLayer;
import net.changed.client.renderer.layers.LatexHumanoidArmorLayer;
...
import net.changed.entity.LimbCoverTransition;
...
import net.changed.util.ReversibleKeyedMap;
import net.changed.util.Transition;
    ...
    public static void renderTransfurringPlayer(Player player, TransfurVariantInstance<?> variant, PoseStack stack, MultiBufferSource buffer, int light, float partialTick) {
        ...
        if (coverProgress >= 1f) {
            findArmorLayer(livingPlayerRenderer).ifPresent(armorLayer -> {
                Arrays.stream(EquipmentSlot.values()).filter(slot -> slot.isArmor()).forEach(armorSlot -> {
                    ...
                    if (item.getItem() instanceof ArmorItem armorItem) {
                        ...
                        if (!layers.isEmpty())
                            texture = net.neoforged.neoforge.client.ClientHooks.getArmorTexture(player, item, layers.get(0), armorSlot == EquipmentSlot.LEGS, armorSlot);
                    }
