...
public abstract class TextMenu extends AbstractContainerMenu implements UpdateableMenu {
    ...
    public TextMenu(MenuType<?> type, int id, Inventory inventory, FriendlyByteBuf extraData) {
        this(type, id, inventory, extraData, false);
    }

    public TextMenu(MenuType<?> type, int id, Inventory inventory, FriendlyByteBuf extraData, boolean readBlockPos) {
        super(type, id);
        this.level = inventory.player.level();
        this.player = inventory.player;
        this.textMenuBlockEntity = null;

        BlockPos clientBlockPos = null;
        BlockState clientBlockState = Blocks.AIR.defaultBlockState();
        if (extraData != null && readBlockPos) {
            clientBlockPos = extraData.readBlockPos();
            clientBlockState = this.level.getBlockState(clientBlockPos);
        }

        this.blockPos = clientBlockPos;
        this.blockState = clientBlockState;
    ...
    public void setText(String text) {
        if (!level.isClientSide && textMenuBlockEntity != null)
            ...
            if (blockPos != null) {
                tag.putLong("TextBlockPos", blockPos.asLong());
            }
            this.setDirty(tag);
        }
    }
