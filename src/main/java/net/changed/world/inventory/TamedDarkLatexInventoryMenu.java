...
import net.changed.entity.ai.DarkLatexInventory;
...
public class TamedDarkLatexInventoryMenu extends AbstractContainerMenu {
    ...
    public EquipmentSlot denyInvalidArmorSlot(ItemStack itemStack) {
        var slot = this.tamedDarkLatex.getEquipmentSlotForItem(itemStack);
        if (!slot.isArmor())
            return slot;
    ...
    public ItemStack quickMoveStack(Player player, int slotId) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            } else if (equipmentslot.isArmor() && !this.slots.get(3 - equipmentslot.getIndex()).hasItem()) { // Move to armor
