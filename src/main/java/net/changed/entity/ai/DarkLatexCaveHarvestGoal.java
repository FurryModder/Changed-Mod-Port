...
public class DarkLatexCaveHarvestGoal extends MoveToBlockGoal {
    ...
    public float getDigSpeed(BlockState blockState, @Nullable BlockPos pos) {
        ...
        if (f > 1.0F) {
            int i = this.getBlockEfficiencyBonus();
        ...
        if (entity.isEyeInFluidType(NeoForgeMod.WATER_TYPE.value()) && !this.hasAquaAffinity()) {
            f /= 5.0F;
        }
    ...
    private int getBlockEfficiencyBonus() {
        var enchantments = entity.registryAccess().lookupOrThrow(Registries.ENCHANTMENT);
        return entity.getMainHandItem().getEnchantmentLevel(enchantments.getOrThrow(Enchantments.EFFICIENCY));
    }

    private boolean hasAquaAffinity() {
        var enchantments = entity.registryAccess().lookupOrThrow(Registries.ENCHANTMENT);
        return EnchantmentHelper.getEnchantmentLevel(enchantments.getOrThrow(Enchantments.AQUA_AFFINITY), entity) > 0;
    }
    ...
    public float getDestroyProgress(BlockState blockState, BlockGetter p_60468_, BlockPos p_60469_) {
        ...
        if (f == -1.0F) {
            ...
            //int i = net.neoforged.neoforge.common.CommonHooks.isCorrectToolForDrops(blockState, player) ? 30 : 100;
    ...
    public void finishMining(ItemStack pickaxe) {
        ...
        if (!blockState.isAir())
            pickaxe.hurtAndBreak(1, entity, EquipmentSlot.MAINHAND);
