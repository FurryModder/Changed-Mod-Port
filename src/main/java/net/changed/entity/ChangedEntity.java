...
import net.changed.mixin.entity.EntityAccessor;
...
import static net.changed.entity.variant.TransfurVariant.findEntityTransfurVariant;
...
public abstract class ChangedEntity extends Monster implements EntityShape.Provider {
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(DATA_TARGET_ID, OptionalInt.empty());
        builder.define(DATA_LOCAL_VARIANT_INFO, new BasicPlayerInfo());
        builder.define(DATA_CHANGED_ENTITY_FLAGS, (byte)0);
    }
    ...
    public @Nullable SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroup) {
    ...
    protected EntityDimensions getDefaultDimensions(Pose pose) {
        ...
        Pose resolvedPose = Objects.requireNonNullElse(overridePose, pose);

        if (WhiteLatexTransportInterface.isEntityInWhiteLatex(this.maybeGetUnderlying()))
            return withChangedEyeHeight(resolvedPose, EntityDimensions.scalable(core.width(), core.width()));

        EntityDimensions dimensions = (switch (resolvedPose) {
            case STANDING -> core;
            ...
            default -> core;
        }).scale(getBasicPlayerInfo().getSize(this));

        return withChangedEyeHeight(resolvedPose, dimensions);
    }

    protected EntityDimensions withChangedEyeHeight(Pose pose, EntityDimensions dimensions) {
        if (pose == Pose.SLEEPING)
            return dimensions;
        return dimensions.withEyeHeight(this.getStandingEyeHeight(pose, dimensions));
    }
    ...
    protected float computeTransfurDamage() {
        ...
        float damage = (float)self.getEntity().getAttributeValue(ChangedAttributes.TRANSFUR_DAMAGE);
    ...
    public static AttributeSupplier.Builder createLatexAttributes() {
        return Monster.createMonsterAttributes().add(ChangedAttributes.TRANSFUR_DAMAGE, 3.0D)
                .add(ChangedAttributes.SPRINT_SPEED, 1.0D)
                .add(ChangedAttributes.SNEAK_SPEED, 1.0D)
                .add(ChangedAttributes.AIR_CAPACITY, 15.0)
                .add(ChangedAttributes.JUMP_STRENGTH, 1.0D)
                .add(ChangedAttributes.FALL_RESISTANCE, 1.0D);
    }
    ...
    protected void setAttributes(AttributeMap attributes) {
        ...
        attributes.getInstance(NeoForgeMod.SWIM_SPEED).setBaseValue(1.0);
        attributes.getInstance(Attributes.ATTACK_DAMAGE).setBaseValue(3.0);
        attributes.getInstance(Attributes.ARMOR).setBaseValue(4.0);
        attributes.getInstance(Attributes.STEP_HEIGHT).setBaseValue(0.7);
        attributes.getInstance(ChangedAttributes.SPRINT_SPEED).setBaseValue(1.0D);
        attributes.getInstance(ChangedAttributes.SNEAK_SPEED).setBaseValue(1.0D);
        attributes.getInstance(ChangedAttributes.AIR_CAPACITY).setBaseValue(15.0);
        attributes.getInstance(ChangedAttributes.JUMP_STRENGTH).setBaseValue(1.0);
        attributes.getInstance(ChangedAttributes.FALL_RESISTANCE).setBaseValue(1.0);
    }
    ...
    public void mirrorLiving(LivingEntity player) {
        ...
        LivingEntityAccessor thisLiving = (LivingEntityAccessor)this;
        LivingEntityAccessor playerLiving = (LivingEntityAccessor)player;
        EntityAccessor thisEntity = (EntityAccessor)this;
        EntityAccessor playerEntity = (EntityAccessor)player;
        ...
        thisLiving.changed$setJumping(playerLiving.changed$getJumping());
        thisEntity.changed$setWasTouchingWater(playerEntity.changed$getWasTouchingWater());
        thisLiving.changed$setSwimAmount(playerLiving.changed$getSwimAmount());
        thisLiving.changed$setSwimAmountO(playerLiving.changed$getSwimAmountO());
        this.forgeFluidTypeOnEyes = player.forgeFluidTypeOnEyes;
        thisLiving.changed$setFallFlyTicks(playerLiving.changed$getFallFlyTicks());

        this.setSharedFlag(7, player.isFallFlying());
        thisEntity.changed$setVehicle(player.getVehicle());
        thisLiving.changed$setUseItemRemaining(playerLiving.changed$getUseItemRemaining());
    ...
    public int getMaxAirSupply() {
        ...
        return this.getAttributes() == null ? 300 : Math.round(20f * (float) this.getAttributes().getValue(ChangedAttributes.AIR_CAPACITY));
    }
