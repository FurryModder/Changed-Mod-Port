...
import net.changed.block.AbstractLargePanel;
import net.changed.block.NineSection;
...
import net.changed.item.BenignShorts;
import net.changed.item.ExoskeletonItem;
import net.changed.item.LaserReactiveItem;
...
public class Exoskeleton extends AbstractRobot {
    ...
    private ItemEnchantments savedEnchantments = ItemEnchantments.EMPTY;
    ...
    public ItemStack getDropItem() {
        ...
        stack.set(DataComponents.ENCHANTMENTS, savedEnchantments);
        return stack;
    }
    ...
    public void loadFromItemStack(ItemStack stack) {
        ...
        savedEnchantments = stack.getOrDefault(DataComponents.ENCHANTMENTS, ItemEnchantments.EMPTY);
    }

    @Override
    public void addAdditionalSaveData(CompoundTag tag) {
        super.addAdditionalSaveData(tag);
        ItemEnchantments.CODEC.encodeStart(NbtOps.INSTANCE, savedEnchantments)
                .result()
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        savedEnchantments = tag.contains("Enchantments")
                ? ItemEnchantments.CODEC.parse(NbtOps.INSTANCE, tag.get("Enchantments"))
                .result()
    ...
    protected EntityDimensions getDefaultDimensions(Pose pose) {
        EntityDimensions dimensions = this.isCharging() && pose == Pose.SLEEPING
                ? super.getDefaultDimensions(Pose.STANDING)
                : super.getDefaultDimensions(pose);
        return this.isCharging() ? dimensions.withEyeHeight(this.getSleepOffset()) : dimensions;
    }
    ...
    public static class ExoskeletonRestrainGoal extends MeleeAttackGoal {
        ...
        protected double getAttackReachSqr(LivingEntity target) {
            ...
            if (exoskeleton.targetHasBenignPants(target))
                return reach * 1.5;
            else return reach * 0.75;
        }

        @Override
        protected boolean canPerformAttack(LivingEntity target) {
            return this.isTimeToAttack() && this.mob.distanceToSqr(target) <= this.getAttackReachSqr(target) && this.mob.getSensing().hasLineOfSight(target);
        }
            ...
            if (target != null) {
                ...
                if (this.exoskeleton.getSensing().hasLineOfSight(target) && exoskeleton.distanceToSqr(target) < 25.0) { // ~5 blocks
                    ...
                    if (this.attackTime == 0) {
                        ...
                        List<ItemUtil.SlottedItem> reactiveItems = ItemUtil.getWearingItems(target, itemStack -> itemStack.getItem() instanceof LaserReactiveItem).toList();
                        reactiveItems.forEach(slottedItem -> {
