...
import net.changed.process.TransfurEvents;
...
public class TransfurVariant<T extends ChangedEntity> {
    ...
    public static Stream<TransfurVariant<?>> getPublicTransfurVariants() {
        return ChangedRegistry.TRANSFUR_VARIANT.get().getValues().stream();
    }
    ...
    public IAbstractChangedEntity replaceEntity(@NotNull LivingEntity entity, @Nullable LivingEntity cause) {
        ...
        if (entity instanceof Player player) {
            ...
            // Take armor
            Arrays.stream(EquipmentSlot.values()).filter(slot -> slot.isArmor()).forEach(slot -> {
                newEntity.setItemSlot(slot, entity.getItemBySlot(slot).copy());
            });
            // Drop held items
