        ...
        public final boolean setupAnimation(ItemStack itemStack, EntityStateContext entity, UpperModelContext model, InteractionHand hand) {
            ...
            if (!(entity.attackTime <= 0.0F)) {
                ...
                setupUsingAnimation(itemStack, entity, model, hand == InteractionHand.MAIN_HAND ?
                        entity.livingEntity.getMainArm() : entity.livingEntity.getMainArm().getOpposite(), 1.0F - (((float)entity.livingEntity.getUseItemRemainingTicks() - entity.partialTicks + 1.0F) / (float)entity.livingEntity.getUseItem().getUseDuration(entity.livingEntity)));
            } else {
