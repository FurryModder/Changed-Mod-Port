...
import net.changed.network.packet.GrabEntityPacket;
import net.changed.process.ProcessTransfur;
import net.changed.util.KeyStatesTracker;
...
public class GrabEntityAbilityInstance extends AbstractAbilityInstance {
    ...
    public void replaceEntityReference(LivingEntity newEntity) {
        ...
        this.grabbedEntityUUID = newEntity.getUUID();
    ...
    public boolean suitEntity(LivingEntity entity) {
        ...
        this.grabbedEntityUUID = entity.getUUID();
        this.suited = true;
    ...
    public void handleEscape() {
        ...
        if (grabbedEntity != null) {
            AttributeInstance grabbedEntityAttribute = grabbedEntity.getAttribute(ChangedAttributes.GRAB_STRUGGLE_STRENGTH);
    ...
    public void tickIdle() { // Called every tick of LatexVariantInstance, for variants that have this ability
        this.resolveGrabbedEntityReference();
    ...
    public void saveData(CompoundTag tag) {
        super.saveData(tag);
        UUID savedGrabbedEntity = grabbedEntity != null ? grabbedEntity.getUUID() : grabbedEntityUUID;
        if (savedGrabbedEntity != null)
            tag.putUUID("GrabbedEntity", savedGrabbedEntity);
    ...
    public void readData(CompoundTag tag) {
        ...
        if (tag.contains("GrabbedEntity")) {
            this.grabbedEntityUUID = tag.getUUID("GrabbedEntity");
            this.resolveGrabbedEntityReference();
        }
        if (this.grabbedEntity != null || this.grabbedEntityUUID != null) {
    ...
    private boolean resolveGrabbedEntityReference() {
        if (this.grabbedEntity != null)
            return true;
        if (this.grabbedEntityUUID == null)
            return false;

        Level level = this.entity.getLevel();
        for (Player player : level.players()) {
            if (player.getUUID().equals(this.grabbedEntityUUID)) {
                this.grabbedEntity = player;
        ...
        var matches = level.getEntities(this.entity.getEntity(), this.entity.getEntity().getBoundingBox().inflate(8.0),
                foundEntity -> foundEntity instanceof LivingEntity && foundEntity.getUUID().equals(this.grabbedEntityUUID));
        if (!matches.isEmpty() && matches.get(0) instanceof LivingEntity livingEntity) {
