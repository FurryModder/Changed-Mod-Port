...
public class Caffeinated extends MobEffect {
    ...
    public boolean applyEffectTick(LivingEntity entity, int amplifier) {
        ...
        final int duration = Optional.ofNullable(entity.getEffect(ChangedEffects.CAFFEINATED)).map(MobEffectInstance::getDuration).orElse(0);
    ...
    public void stackEffect(LivingEntity user, int tickDuration, int upgradeTickThreshold) {
        final var existing = user.getEffect(ChangedEffects.CAFFEINATED);
        if (existing != null) {
            if (existing.getDuration() > upgradeTickThreshold)
                user.addEffect(new MobEffectInstance(ChangedEffects.CAFFEINATED, tickDuration, existing.getAmplifier() + 1));
            else
                user.addEffect(new MobEffectInstance(ChangedEffects.CAFFEINATED, tickDuration, existing.getAmplifier()));
        }

        user.addEffect(new MobEffectInstance(ChangedEffects.CAFFEINATED, tickDuration));
    }
}
