...
const float WAVE_EFFECT_MINIMUM = 16.0f;
const float WAVE_BASE_VISIBILITY = 0.08f;
...
vec4 waveVision(vec4 colorIn, float effectValue, float depth, vec3 resonanceColor) {
    float stableEffectValue = max(effectValue, WAVE_EFFECT_MINIMUM);
    vec4 waveColor = waveColoring(colorIn);
    vec4 resoColor = resonanceColoring(colorIn);
    float baseVisibility = WAVE_BASE_VISIBILITY * (1.0f - smoothstep(48.0f, 96.0f, depth));
    float waveFast = max(waveStrength(stableEffectValue, depth, WAVE_DECAY, WAVE_INTERVAL, WAVE_SPEED), baseVisibility);
    float waveRecv = waveStrength(stableEffectValue, depth, WAVE_DECAY_RESONANT, WAVE_INTERVAL, WAVE_SPEED) * colorIntensity(resonanceColor) * 2.0f;
